    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6 p-6">
                ...
                <p class="mt-2 text-gray-500">{{ __('Log an item that you have lost') }}</p>
            </div>
            <div class="rounded-2xl border border-gray-200 bg-white p-6 md:p-8 shadow-sm">
                
                <div class="mb-6">
                    <h2 class="text-lg font-semibold text-gray-900">{{ __('Lost Item Details') }}</h2>
                    <p class="mt-1 text-sm text-gray-500">{{ __('Provide accurate information to help the owner claim it') }}</p>
                </div>

                <form action="{{ route('user.report-lost.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        <label for="location" class="block text-sm font-medium text-gray-700 mb-2">
                            {{ __('Last Seen Location') }} <span class="text-gray-900">*</span>
                        </label>
